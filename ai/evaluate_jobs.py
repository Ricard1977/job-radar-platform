...
EVALUATOR_VERSION=f'gemini:{MODEL}:profile-{PROFILE_VERSION}_criteria-{CRITERIA_VERSION}:spanish-v1'
...
LIST_KEYS=['strongest_matches','gaps_or_risks','hard_barriers','evidence_from_job']; REQUIRED=SCORE_KEYS+['decision','short_reason','spanish_summary']+LIST_KEYS
...
def validate(ev):
 ...
 ev['short_reason']=str(ev['short_reason']).strip(); ev['spanish_summary']=str(ev['spanish_summary']).strip(); return ev
...
def evaluate(api_key,profile,criteria,row):
 prompt='''Eres el evaluador de un radar laboral. Evalúa el encaje, pero no decidas por el candidato. Sigue exactamente el perfil y los criterios. Lee la descripción completa, no solo el título. Distingue falta de evidencia de incompatibilidad y no inventes datos. IMPORTANTE: short_reason, spanish_summary, strongest_matches, gaps_or_risks, hard_barriers y evidence_from_job deben redactarse SIEMPRE en castellano, aunque la oferta esté en otro idioma. spanish_summary debe resumir en 3-5 frases qué haría la persona, responsabilidades y requisitos clave. Devuelve solo el JSON solicitado.\n\nINPUT:\n'''+json.dumps({'candidate_profile':profile,'evaluation_criteria':criteria,'job':compact_job(row)},ensure_ascii=False)
 schema={'type':'OBJECT','properties':{'score':{'type':'NUMBER'},'decision':{'type':'STRING','enum':['HIGH_INTEREST','INTERESTING','REVIEW','LOW_INTEREST']},'responsibility_and_seniority':{'type':'NUMBER'},'project_program_delivery':{'type':'NUMBER'},'engineering_technical_environment':{'type':'NUMBER'},'leadership_and_stakeholders':{'type':'NUMBER'},'sector_domain_transferability':{'type':'NUMBER'},'critical_infrastructure_availability':{'type':'NUMBER'},'short_reason':{'type':'STRING'},'spanish_summary':{'type':'STRING'},'strongest_matches':{'type':'ARRAY','items':{'type':'STRING'}},'gaps_or_risks':{'type':'ARRAY','items':{'type':'STRING'}},'hard_barriers':{'type':'ARRAY','items':{'type':'STRING'}},'evidence_from_job':{'type':'ARRAY','items':{'type':'STRING'}}},'required':REQUIRED}
...
def main():
 ...
 for row in rows:
  try:
   ev=evaluate(api_key,profile,criteria,row);ts=now();detail={'short_reason':ev['short_reason'],'spanish_summary':ev['spanish_summary'],'evidence_from_job':ev['evidence_from_job'],'profile_version':PROFILE_VERSION,'criteria_version':CRITERIA_VERSION,'provider':'google-gemini','api_model':MODEL}
